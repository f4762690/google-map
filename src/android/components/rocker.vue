...
export default {
    ...
    methods:{
        ...
        movePoint:function(ev)
        {
            if(this.startDrag)
            {
                ...
                this.angle = this.getAngle(this.rocker_x,this.rocker_y,ev.pageX,ev.pageY);
            }
        },
        ...
        ishold:function(val)
        {
            ...
            if(true == val)
            {
                let player = ()=>{
                    ...
                    this.moveing = true
                    this.move(Math.sin(__j)*this.step,Math.cos(__j)*this.step)
                    this.autoIntervalId=setTimeout(player,500)
                }
                player()
            }
    ...
    created:function()
    {
        ...
        const keybordMover = ()=>{
            this.checkKey();
          this.keymoverTimeoutID = setTimeout(keybordMover,500);
        }
        keybordMover();
