<template>
    <div draggable="false" class="speed-wrap" @wheel="wheelHandler">
        ...
        <p draggable="false" class="speed-content">{{trans.speed}}: <span>{{speed*2/100}}m/s {{speed_km}}km/h</span></p>
    </div>
</template>
...
export default {
    ...
    computed:{
        ...
        ...mapGetters(['trans','cycleModel','speed','maxSpeed'])
    },
    // mounted:function()
    // {
    ...
    watch:{
      loopData(newValue){
        this.$store.commit('setSpeed',this.maxSpeed*(newValue/this.maxRange));
        this.syncSpeed()
      },
      speed(newValue){
       this.loopData = (newValue/this.maxSpeed)*this.maxRange;
      }
    },
    methods:{
        move:function(ev)
