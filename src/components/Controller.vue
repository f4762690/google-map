<template>
  <div id="app" @drop="gpxfileloader" @dragover="dragoverHandler" @dragenter="dragenterHandler" >
    ...
    <div id="pac-input" class="controls" v-show="isMapInit" @mouseleave="history_hide">
        ...
        <history @editHistory="editHistory" ref="history" @importFile="gpxfileloaderMethod"></history>
    ...
    <div id="con-group" v-show="isMapInit">
        <btn :action="!isStop ? stopSimulation : ()=>{}":style="isStop ? {background:'#ccc'} : ''">{{trans.stopSim}}</btn>
        <map_btn :icon="p_to_p" :type="0" :simulationType="simulationType"></map_btn>
        <map_btn :icon="line" :type="1" :simulationType="simulationType"></map_btn>
        <map_btn :icon="m_point"  :type="2" :simulationType="simulationType"></map_btn>
    </div>
    ...
    <!--速度控制-->
    <speedBar id="speedBar" ref="speedBar"></speedBar>
...
export default {
  name: 'App',
  components:{ run_message, rocker, speedBar, tip, history, historyAdd, MPB, cloud, subscribe },
  data(){
    return {
        showSpeed:false,
        //地图是否加载完成
  ...
  computed:{
    ...
    ...mapGetters(['license','subinfo','hasMoreWin','speed','isFavorite','cycleModel','runPause','trans','isStop', 'wakeSpeed', 'rockerSpeed', 'morePointSpeed', 'TrialTime', 'Remaining', 'version', 'my_location', 'repeatModel', 'userToken'])
  },
  ...
  mounted:function(){
    ...
    this.$addQuickKey(['ctrl','n4'],()=>{
        let _speed = this.speed > 100 ? this.speed - 1 : 100
        this.$store.commit('setSpeed',_speed)
    })
    this.$addQuickKey(['ctrl','n5'],()=>{
        let _speed = this.speed < 1000 ? this.speed + 1 : 1000
        this.$store.commit('setSpeed',_speed)
    })
  },
  ...
  methods:{
    moreWin:function(){
        if(window.osType == 'pc'){
            try{
                window.LaunchPlugin("locationMultiDev")
            }catch(error){}
        }

        if(window.osType == 'mac'){
            try{
                window.external.launchVLPro()
            }catch(error){}
        }
    },
    //显示gpx搜索
    ...
    readGPX:function(result)
    {
        ...
        if(_metadata.length > 0)
        {
            ...
            if(_speed)
            {
                _speed = _speed > 1000 ? 1000 : _speed < 100 ? 100 : _speed
                this.$store.commit('setSpeed',_speed)
            }
            if(_loop)
            {
    ...
    */
    ...
    __wakeSimulateion:function(obj){
        ...
        //所需时间
        this.run_total_time     = Math.ceil(this.roule.distance / this.speed)
        //总距离
        ...
        let __first = runer.getPoints()[0]

        const __run = ()=>{
            let point = runer.getNextPoint(5)
            console.log(runer.getProgress())
            //固定时间调用客户端api
            if(point)
            {
                //路径大于1继续行走
                this.progress++
                this.map.setCurMarketPosition(point)
                this.NativeSimulateLocationTo(point)
                this.runIntervalId = setTimeout(__run,500)
            }else{
                // //行走完成后，判断重复次数
                if(this.cycleModel > 0)
                {
                    //重置进度
                    this.progress = 0
                    ...
                    __first = runer.getPoints()[0]

                    //如果不是无限循环,则递减循环次数
                    if(this.cycleModel < 999)
                    {
                        this.$store.commit('setCycleModel',this.cycleModel-1);
                    }
    ...
    wakeSimulateion:function(obj)
    {
        this.line_distance      = this.roule.line_distance
        this.point_list         = this.roule.crude
        this.run_total_time     = this.roule.time
        this.run_total_distance = this.roule.distance
        ...
        if(this.point_list.length>0)
        {
            ...
            let __obj = obj || this.point_list[0];
            const play = ()=>{
                //是否是暂停状态
                if(this.runPause)
                {
                    if(this.progress < this.point_list.length)
                    {
                        this.runIntervalId = setTimeout(play,this.speed);
                    }
                    return;
                }
                ...
                if(this.progress >= this.point_list.length)
                {
                    ...
                    if(this.cycleModel > 0)
                    {
                        ...
                        if(this.repeatModel)
                        {
                            this.point_list.reverse();
                        }else{
                            if(__obj.lat != this.point_list[0].lat && __obj.lng != this.point_list[0].lng)
                            {
                                this.point_list.reverse();
                            }
                        }

                        this.wakeSimulateion(__obj);
                        if(this.cycleModel < 999)
                        {
                            this.$store.commit('setCycleModel',this.cycleModel-1);
                        }
                    ...
                    let point = this.point_list[this.progress];
                    this.map.setCurMarketPosition(point)
                    this.NativeSimulateLocationTo(point);
                    this.runIntervalId = setTimeout(play,this.speed);
                }
