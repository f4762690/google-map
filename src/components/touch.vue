...
export default {
    ...
    methods: {
        touchmove: function(target)
        {
            if(this.m)
            {
                this.pos.left = Math.floor(target.pageX - this.disX) + 'px';
                this.pos.top = Math.floor(target.pageY - this.disY) + 'px';
            }
        },
        touchstart: function(ev)
        {
            this.disX = Math.floor(ev.pageX - this.$refs.touch.offsetLeft);
            this.disY = Math.floor(ev.pageY - this.$refs.touch.offsetTop);
            this.m=true;
        },
        touchend: function()
        {
